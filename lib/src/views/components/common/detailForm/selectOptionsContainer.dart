...
import 'package:opalus/src/views/components/tag/addTagButton/addTagButton.dart';
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
      ...
      builder: (context, AsyncSnapshot<DetailFormState> snapshot) {
        ...
        var tapped = isGroup ? Map.of(state.groupsSelected) : Map.of(state.tagsSelected);
        List<Card> mappedModel = models
            .map((model) => Card(
                  margin: EdgeInsets.all(0),
                  color: tapped[model.id] == true ? MyTheme.primaryColorLight() : Colors.white,
                  child: InkWell(
                    onTap: () {
                      ...
                      String newValue = isSelected
                          ? curValue.replaceAll(", $newName", '').replaceAll("$newName, ", '').replaceAll(newName, '')
                          : curValue != ''
                              ? "$curValue, $newName"
                              : newName;
                      ...
                      DetailFormEvent event = isGroup ? DetailFormTap<Group>(tapped) : DetailFormTap<Tag>(tapped);
        ...
        List<Widget> items = [...mappedModel, AddTagButton()];
